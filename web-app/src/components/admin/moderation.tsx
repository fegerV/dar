...
export function AdminModeration() {
  ...
  const { items, loading, page, pageSize, total, totalPages, setPage, setPageSize, setFilters } = useAdminList<ModerationItem>({
                ...
                {items.map((item) => (
                  <tr key={item.id} className="border-b">
                    ...
                    <td className="py-2 text-right">
                      <Button size="sm" variant="ghost" aria-label={`View ${item.id}`} onClick={() => router.push(`/admin/moderation/${item.id}`)}>View</Button>
                    </td>
                  </tr>
                ))}
                {items.length === 0 && (
